...
invisible(parallel::mclapply(seq_len(nrow(todo)), function(i) {
  ...
  r <- do.call(rbind, lapply(seq_len(N_SIM), function(k) {
    ...
    z <- tryCatch(fit_all(draw(cc)), error = function(e) NULL)
    if (is.null(z)) NULL else data.frame(t(z), rep = k) }))
  r$cell <- cc$cell; r$truth <- truth(cc); r$truth_r <- truth_restricted(cc)
