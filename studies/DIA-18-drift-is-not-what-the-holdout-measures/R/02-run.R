...
stamp <- list(code_mtime = file.mtime("R/00-model.R"), versions = vapply(c("multinma", "netmeta", "metadat"), function(p) as.character(utils::packageVersion(p)), ""), R = R.version.string)
invisible(parallel::mclapply(seq_len(nrow(todo)), function(i) {
  cc <- todo[i, ]; set.seed(MASTER_SEED %% 1e6 + 104729L * cc$cell)
  r <- tryCatch(run_cell(cc), error = function(e) list(error = conditionMessage(e)))
