...
k <- merge(merge(mm[, c("cell", "ratio", "rmse")], mv[, c("cell", "rmse")], by = "cell", suffixes = c("_mm", "_mv")),
           setNames(mi[, c("cell", "rmse")], c("cell", "rmse_mi")), by = "cell")
lo <- k$ratio == 0.5
...
`r sum(k$rmse_mv[lo] > k$rmse_mm[lo])` of 6 scenarios. With the source more dispersed, it
cost little: balancing the index variance lowered RMSE in
`r sum(k$rmse_mi[!lo] < k$rmse_mm[!lo])` of 6 scenarios and balancing every variance in
`r sum(k$rmse_mv[!lo] < k$rmse_mm[!lo])`, the rest within 0.01. Balancing only the estimated index variance kept more
