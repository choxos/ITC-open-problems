...
CMP-14 restricted. No change was made to the design after any result was seen.

### 2026-07-29, after a first run: power-scaling is measured distributionally

**What changed.** The power-scaling diagnostic was implemented as the shift in the posterior
*mean* per unit log alpha. It is now the Hellinger distance between the base and power-scaled
posteriors per unit log alpha, which is distributional.

**Why.** In a conjugate Gaussian with a zero-centered prior, raising the prior to the power
alpha and lowering the likelihood to the power 1/alpha give posteriors with exactly the same
mean, since the precision is $G + \alpha P$ either way. A mean-shift sensitivity therefore
reports prior and likelihood sensitivity as *identical by construction*, and a rule of the
form "prior sensitive and likelihood insensitive" can never fire. Measured that way the
diagnostic had sensitivity 0.000 in every scenario, and publishing that would have reported a
property of our implementation as a property of the method.

The posterior standard deviations do differ, so the two are distinguishable
distributionally, and that is how Kallioinen and colleagues define the diagnostic: a
divergence between base and power-scaled posteriors, in their case cumulative Jensen-Shannon
distance. The Hellinger distance is exact in closed form for Gaussians and lies on the same
[0, 1] scale, so their default 0.05 threshold carries over unchanged.

**What it does not change.** No factor, no scenario, no reference standard, no threshold, and
no other diagnostic. The first run's output was discarded and the design rerun in full.
