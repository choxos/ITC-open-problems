...
| **round 1 of critique** | **THE HEADLINE IS WITHDRAWN. P3 compared two gradients in different coordinates.** | `delta_gradient()` differentiates the estimand with respect to means and **SDs**. `estimator_gradient()` differentiates with respect to means and **raw second moments**, because `h = cbind(x, x^2)` and the reported moment vector is `c(colMeans(x), colMeans(x^2))`. `Omega_normal()` is the covariance of means and raw second moments. P3 compared the first with the second and multiplied the first by the third, with no Jacobian. Verified: the reported moment vector is (1.1936, 1.1685, 1.0508) where the SDs are (1.0184, 1.0068, 0.9558), and transforming the gradient correctly changes the reference variance by a factor of **0.7465** at one setting. **The factor differs by link, which is what manufactured the apparent link-specific direction.** So "anti-conservative on logit, conservative on cloglog" is not established, and neither is the 38% gradient gap |
| **round 1** | **The identity control could not have caught it**, which is why it ran clean | Under the identity link the estimand's SD-gradient is exactly zero, so the mismatched coordinate contributes nothing and the control passes whatever the coordinate error is. A control that is blind to the defect it is positioned to catch is worse than no control, because it was cited as establishing that the curved-link result was about curvature rather than implementation |
| **round 1, corrected** | **The corrected answer largely favours the catalog, not this study.** | With both gradients in (mean, raw second moment) coordinates: the worst relative gradient gap falls from **0.379 to 0.057**, and the variance ratio from **0.764-1.438 to 0.989-1.174**. On `logit`, the study's PRIMARY arm, the ported variance is **essentially correct** at 0.989 to 1.074. On `cloglog` it is anti-conservative by 8% to 17%. **The direction no longer differs by link**: nothing is conservative anywhere. The reviewer predicted the corrected reference variances to three figures, 8.125e-05 and 4.147e-05, and both reproduce exactly |
| **round 1** | **The shared target-trial variance was fourfold too small**, so every interval was too narrow for a reason unrelated to the study | `var_theta_BC()` assumed equal arms at $p = 0.5$. Measured against simulation it understated the variance by **1.6x on identity, 3.7x on logit and 4.25x on cloglog**. Every method carries the term, so all of them would have undercovered and both the primary comparison and the identity falsifier would have been invalid. It is computed from the realized target arms now, which is what a published trial reports, and the ratios are 1.05, 0.82 and 1.10 |
| **round 1** | **The registered decision rule now points the other way on the primary arm.** | Section 5 registers that if the entropy port restores nominal coverage on the logit scale the catalog is right and prediction 2 is withdrawn. A ported variance within 1% to 7% of correct on that scale is not obviously going to break coverage, so **the outcome the design registered as the one that would most weaken its headline is now the likely one**. That is worth stating before the run rather than after |
| **P3** | The expected headline appeared to survive, **on the mismatched comparison** | On curved links the ported gradient is wrong by up to **38% relative**, 22.9 Monte Carlo SE, and the variance it produces is off by a factor of **0.76 to 1.44**. The ports are anti-conservative on logit, by up to 44%, and conservative on cloglog, by up to 24%. **The direction differs by link**, which no part of the design predicted |
